...
* Break up long lines with trailing dot-notation [Example][dot notation example]
...
* Prefer `&&` and `||` over `and` and `or`. (Be aware Ruby has different precedence rules for `and` and `&&`
  and there are cases when `and` is the only option [Example][render example])
* Prefer `!` over `not`.
...
* Prefer `protected` over `private` for non-public `attr_reader`s, `attr_writer`s,
  and `attr_accessor`s. [Example][protected attributes example]
* Order class methods above instance methods. [Example][class method example]
* Leave an empty line at the end of a file

[class method example]: /samples/ruby.rb#L4
[required kwargs]: /samples/ruby.rb#L28
[trailing comma example]: /samples/ruby.rb#L61
[protected attributes example]: /samples/ruby.rb#L108
[class definition example]: /samples/ruby.rb#L121
[render example]: /samples/ruby.rb#L128
[dot notation example]: /samples/ruby.rb#L133
